  ...
  describe "when included into a class" do
    ...
    describe "#read" do
      it "caches reads" do
        ...
        expect(backend.spy).to receive(:read).once.with(currency, options).and_return("foo")
        2.times { expect(backend.read(currency, options)).to eq("foo") }
      end

      it "does not cache reads with cache: false option" do
        backend = cached_backend_class.new("model", "attribute")
        expect(backend.spy).to receive(:read).twice.with(currency, options).and_return("foo")
        2.times { expect(backend.read(currency, options.merge(cache: false))).to eq("foo") }
      end

      it "does not modify options passed in" do
        backend = cached_backend_class.new("model", "attribute")
        allow(backend.spy).to receive(:read).with(currency, {}).and_return("foo")
        options = { cache: false }
    ...
    describe "#write" do
      it "returns value fetched from backend" do
        backend = cached_backend_class.new("model", "attribute")
        expect(backend.spy).to receive(:write).twice.with(currency, "foo", options).and_return("bar")
        2.times { expect(backend.write(currency, "foo", options)).to eq("bar") }
      end

      it "stores value fetched from backend in cache" do
        backend = cached_backend_class.new("model", "attribute")
        expect(backend.spy).to receive(:write).once.with(currency, "foo", options).and_return("bar")
        expect(backend.write(currency, "foo", options)).to eq("bar")
        expect(backend.spy).not_to receive(:read)
        expect(backend.read(currency, options)).to eq("bar")
      end

      it "does not store value in cache with cache: false option" do
        backend = cached_backend_class.new("model", "attribute")
        allow(backend.spy).to receive(:write).once.with(currency, "foo", options).and_return("bar")
        expect(backend.write(currency, "foo", options.merge(cache: false))).to eq("bar")
      ...
      it "does not modify options passed in" do
        backend = cached_backend_class.new("model", "attribute")
        allow(backend.spy).to receive(:write).with(currency, "foo", {})
        options = { cache: false }
        backend.write(currency, "foo", options)
        expect(options).to eq({ cache: false })
      end
    end
